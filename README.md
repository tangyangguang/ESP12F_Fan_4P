...
- `http://esp-fan.local/ir`
- `http://esp-fan.local/esp8266base`
- `http://esp-fan.local/logs`
- `http://esp-fan.local/system`
- `http://esp-fan.local/health`

默认 Web 登录用户为 `admin`，默认密码为 `admin123`。密码可以在 `/system` 页面进入 Auth Password 修改。

## 功能

- 0-100% PWM 调速，25 kHz 输出，`analogWriteRange(255)`，GPIO5 按开漏下拉驱动反相写入，支持最低有效转速限制。
- 四线风扇转速反馈采集，支持基于 RPM=0 持续时间的堵转检测和保护。
- 软启动、软停止，时间可配置。
- 定时运行，支持常用预设和自定义时间。
- 本地加速/减速按键控制。
- 红外遥控学习，支持加速、减速、停止、30min/1h/2h/4h/8h 定时快捷键，以及 1-4 档直达快捷键。
...
风扇 PWM 占空比以风扇 PWM 引脚实测波形为准：0% 为停止，100% 为全速。实机验证时建议用示波器或逻辑分析仪确认 25 kHz，以及 0/25/50/75/100% 是否与 Web 设定一致。
...
`/fan` 是主控制页，显示状态和简洁原因（例如 `Error / Blocked - no RPM feedback`）、目标速度、实际输出、档位、RPM、定时倒计时、累计总运行时长、本次启动后运行时长和 RSSI。两个运行时长都只读：累计总运行时长会低频持久化，断电最多回退 `fan_runtime_save_min`；本次启动后运行时长只保存在内存中，设备重启后归零。页面提供速度预设、自定义速度、定时预设、自定义定时、取消定时和停止风扇操作。

`/config` 是配置页，可修改最低速度、休眠等待时间、软启动、软停止、堵转检测时间、操作反馈闪烁时长、运行状态保存间隔和上电恢复策略。配置保存成功会显示 changed 数；无变化时仅显示 no changes，不触发 LED 操作反馈。

`/ir` 是红外学习页，由服务端分块渲染所有命令，并显示每个命令是否已学习、协议号和红外码；每个命令可单独学习或清除。同一个红外码只能绑定一个命令，已有重复绑定会标记为 Duplicate。当前命令包括加速、减速、停止、30min/1h/2h/4h/8h 定时，以及 1-4 档直达（25%/50%/75%/100%）。
...
`/system` 是 Esp8266Base 内置维护页，统一提供 WiFi Settings、Auth Password、OTA Update、FileLog、Clear logs 和 Reboot 等低频维护入口。外层内置导航只保留 Status / Logs / System。

`/wifi` 是 Esp8266Base 内置 WiFi 配网页，可查看并保存 STA 凭证；无已保存凭证时设备会进入 AP 配网模式。该入口从 `/system` 进入。

`/ota` 是 Esp8266Base 内置 OTA 上传页，当前固件已通过 `ESP8266BASE_USE_OTA=1` 启用。GET 页面和 POST 上传都使用同一组 Basic Auth；上传页显示进度，上传期间基础库会配合 Watchdog 暂停/恢复，成功后重启。该入口从 `/system` 进入。

`/auth` 是 Esp8266Base 内置改密页，用于修改 Web Basic Auth 密码。该入口从 `/system` 进入。

`/logs` 使用 Esp8266Base 内置日志页面，读取 `/logs/app.log` 及滚动日志文件。一次新启动会有启动分割和启动次数，NTP 同步后日志时间使用真实日期时间。

`/reboot` 只保留 POST 重启动作，不再提供 GET 页面；重启由 `/system` 页面中的表单触发。`/health` 是无需认证的 JSON 健康检查入口。
...
| `/api/ir/learn` | POST | 启动红外学习，POST 参数 `key_index=0..11`；清除红外码使用 `key_index=0..11&clear=1` |
...
| `fan_runtime_save_min` | `1` | 运行状态持久化节流间隔，范围 1-60 分钟；值越大越省 Flash，但断电后定时剩余时间和累计运行时长最多回退该间隔 |
| `fan_auto_restore` | `true` | 上电后是否恢复上次速度和定时 |

运行状态还会持久化上次速度、剩余定时、累计运行时长和红外学习码。运行中的周期性运行状态保存使用 `fan_runtime_save_min` 节流；调速、定时、停止等关键操作仍会立即安排保存。升级到可配置节流后，默认周期性保存间隔从旧版约 5 秒调整为 1 分钟，以减少 Flash 写入。Web 密码由 Esp8266Base 使用 `eb_web_pass` 管理。
...
-DESP8266BASE_DEFAULT_HOSTNAME=\"esp-fan\"
-DESP8266BASE_FILELOG_DEFAULT_MODE=ESP8266BASE_FILELOG_MODE_WARN
-DESP8266BASE_FILELOG_PATH=\"/logs/app.log\"
-DESP8266BASE_FILELOG_MAX_BYTES=16384
-DESP8266BASE_FILELOG_ROTATE_FILES=4
