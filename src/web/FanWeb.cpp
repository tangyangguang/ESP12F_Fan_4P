...
static const char CONFIG_IR_END[] PROGMEM =
    ...
    "function watchIr(i,n,seq,rej){fetch('/api/status').then(r=>r.json()).then(j=>{var d=j.data;if(!d)return;if(d.ir_learn_seq!=seq){var v='Protocol '+d.ir_last_protocol+' - '+d.ir_last_code;setIrRow(i,v);setIr('Learned '+n+' - '+v,'oktxt')}else if(d.ir_learning&&d.ir_duplicate_key<irNames.length){rej=d.ir_reject_seq;setIr('Already assigned to '+irName(d.ir_duplicate_key)+'. Press a different key - '+d.ir_remaining+'s','errtxt');setTimeout(()=>watchIr(i,n,seq,rej),500)}else if(d.ir_reject_seq!=rej){setIr('Already assigned to '+irName(d.ir_duplicate_key)+'. Press a different key','errtxt')}else if(d.ir_learning){setIr('Learning '+n+' - '+d.ir_remaining+'s','muted');setTimeout(()=>watchIr(i,n,seq,rej),500)}else{setIr('Learn timeout - no valid signal','errtxt')}}).catch(()=>setIr('Learn status failed','errtxt'))}"
