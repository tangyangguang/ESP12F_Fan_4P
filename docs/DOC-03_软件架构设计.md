...
| 堵转复位 | 保护触发后，任意控制源发送启动指令可触发恢复尝试（1.5s 内检测转速） |
...
| SYS_ERROR | 故障状态 | 堵转检测触发 / 硬件初始化失败 / 配置损坏 | 手动复位 / 自动重启 |
...
stateDiagram-v2
    ...
    FAN_STATE_BLOCKED --> FAN_STATE_IDLE : 手动复位
```
...
| FAN_STATE_BLOCKED | 堵转保护 | 堵转检测触发 | 手动复位 |
