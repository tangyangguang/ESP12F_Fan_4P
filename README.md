...
| `fan_runtime_save_min` | `1` | 运行状态持久化节流间隔，范围 1-60 分钟 |
| `fan_auto_restore` | `true` | 上电后是否恢复上次速度和定时 |

运行状态还会持久化上次速度、剩余定时、累计运行时长和红外学习码。运行中的周期性运行状态保存使用 `fan_runtime_save_min` 节流；调速、定时、停止等关键操作仍会立即安排保存。Web 密码由 Esp8266Base 使用 `eb_web_pass` 管理。
