...
`/config` 是配置页，可修改最低速度、休眠等待时间、软启动、软停止、堵转检测时间、操作反馈闪烁时长和上电恢复策略，也提供红外学习入口。红外学习区列出所有命令，并显示每个命令是否已学习、协议号和红外码。配置保存成功会显示 changed 数；无变化时仅显示 no changes，不触发 LED 操作反馈。
...
| 路径 | 方法 | 作用 |
|------|------|------|
| `/api/status` | GET | 获取风扇状态、网络状态、时钟、运行信息和红外命令学习状态 |
...
由于 Esp8266Base 当前配置的应用 API 配额为 6 个，本项目只注册以上 6 个 API。红外学习状态由学习请求结果、`/api/status` 中的 `ir_learn_seq`、`ir_last_*` 和 `ir_keys` 观察，不单独占用一个公开 API；学习序号变化表示本次学习完成。
