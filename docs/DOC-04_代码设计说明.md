...
1. **转速控制**：0-100% 无级调节；非零设置值<最低有效转速自动提升；最低有效转速运行中调高时，低于新阈值的非零目标会提升到新阈值；非零目标变化按软启动时间从当前输出平滑渐变，停止按软停止时间平滑渐变
...
5. **定时功能**：倒计时结束自动停止，定时过程中可随时取消或修改，新设置直接覆盖旧定时；页面提供 30min/1h/2h/4h/8h 预设，自定义最大 99 小时
...
public:
    ...
    bool begin();
    void tick();  // 每帧调用，处理非零目标渐变、软停止渐变和堵转检测
...
- `LedIndicator` 只维护当前档位、基础模式、闪烁窗口、反馈时长和输出状态；`flashOnce()` 是临时反相覆盖，结束后恢复当时最新的基础状态。
...
- 所有有效用户操作闪 1 下：Web 调速/停止/定时/配置保存且有变化、按键有效加减档、IR 有效操作、红外学习成功。反馈采用反相脉冲，当前亮时短暂熄灭、当前灭时短暂点亮，保证 100% 亮度下仍可见。非法 API 参数、配置无变化、边界重复加减档、未知红外码、学习超时不闪。
