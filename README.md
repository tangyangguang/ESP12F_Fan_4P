...
`/fan` 是主控制页，显示状态（堵转会合并显示为 `Error / Blocked`）、目标速度、实际输出、档位、RPM、定时倒计时、累计总运行时长、本次启动后运行时长和 RSSI。两个运行时长都只读：累计总运行时长会低频持久化，断电最多回退 `fan_runtime_save_min`；本次启动后运行时长只保存在内存中，设备重启后归零。页面提供速度预设、自定义速度、定时预设、自定义定时、取消定时和停止风扇操作。
