...
IREvent IRReceiverDriver::getEvent() {
    ...
    if (_ignore_until_tick != 0) {
        if ((int32_t)(millis() - _ignore_until_tick) < 0) {
            ESP8266BASE_LOG_D("IR", "Ignored debounced IR frame: protocol=%d, code=0x%08llX",
                              _last_protocol, static_cast<unsigned long long>(_last_code));
            return IR_EVENT_NONE;
        }
        _ignore_until_tick = 0;
        ...
        if (_protocols[i] == protocol && _codes[i] == code && _codes[i] != 0) {
            switch (i) {
                ...
                case IR_KEY_TIMER_4H:
                    ESP8266BASE_LOG_D("IR", "Matched: TIMER_4H");
                    return IR_EVENT_TIMER_4H;
                case IR_KEY_TIMER_8H:
                    ESP8266BASE_LOG_D("IR", "Matched: TIMER_8H");
                    return IR_EVENT_TIMER_8H;
                case IR_KEY_GEAR_1:
                    ESP8266BASE_LOG_D("IR", "Matched: GEAR_1");
                    return IR_EVENT_GEAR_1;
                case IR_KEY_GEAR_2:
                    ESP8266BASE_LOG_D("IR", "Matched: GEAR_2");
                    return IR_EVENT_GEAR_2;
                case IR_KEY_GEAR_3:
                    ESP8266BASE_LOG_D("IR", "Matched: GEAR_3");
                    return IR_EVENT_GEAR_3;
                case IR_KEY_GEAR_4:
                    ESP8266BASE_LOG_D("IR", "Matched: GEAR_4");
